...
import com.anshtya.jetx.shared.database.entity.toExternalModel
...
import com.anshtya.jetx.shared.profile.model.CreateProfileRequest
import com.anshtya.jetx.shared.profile.model.NetworkProfile
import com.anshtya.jetx.shared.profile.model.toEntity
import com.anshtya.jetx.shared.profile.model.toExternalModel
import com.anshtya.jetx.shared.util.Constants.MEDIA_STORAGE
import com.anshtya.jetx.shared.util.Constants.PROFILE_TABLE
...
class ProfileRepositoryImpl(
    ...
    override suspend fun createProfile(
        ...
        return runCatching {
            ...
            var profilePictureBucketPath: String? = null

            if (profilePicturePath != null) {
                val imageByteArray = imageCompressor.compressImage(profilePicturePath).getOrThrow()
                val path = "profile-${userId}.png"
                mediaBucket.upload(
                    path = path,
                    data = imageByteArray
                )
                profilePictureBucketPath = mediaBucket.publicUrl(path)
            }
            ...
            profileTable.insert(
                value = CreateProfileRequest(
                    name = name,
                    username = username,
                    profilePictureUrl = profilePictureBucketPath
                )
            )
            saveProfile(
                UserProfileEntity(
                    ...
                    name = name,
                    username = username,
                    profilePicture = profilePictureBucketPath
                )
            )
